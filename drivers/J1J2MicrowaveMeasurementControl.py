...
class PowerSweep(StoppableThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.driver.parent.devices[self.microwave['device']].commands.append(f'SetFrequency({microwave_frequency})')
            self.driver.parent.devices[self.microwave['device']].commands.append(f'SetSweepTimeStep({self.microwave["time_step"]})')
            ...
            while True:
                for la_power in self.laser['powers']:
                    self.driver.parent.devices[self.laser['device']].commands.append(self.laser['cmd'].replace('arg', str(la_power)))
                    ...
                    for mu_power in self.microwave['powers']:
                        ...
                        self.driver.parent.devices[self.microwave['device']].commands.append(self.microwave['cmd'].replace('arg', str(mu_power)))
                        # disable the sweep trigger
                        self.driver.parent.devices[self.microwave['device']].commands.append('SetTriggerMode("disabled")')
                        # generate the sweep list, starting at high powers
                        sweep_list = f"LdL0f{microwave_frequency/1e6}L0a{mu_power}L1f{microwave_frequency/1e6}L1a{mu_low}"
                        self.driver.parent.devices[self.microwave['device']].commands.append(f'synth._write({sweep_list})')
                        # set the trigger to activate the full frequency sweep
                        # activated when trigger is pulled low
                        self.driver.parent.devices[self.microwave['device']].commands.append('SetTriggerMode("full frequency sweep")')
...
class J1J2MicrowaveMeasurementControl:
    ...
    def __init__(self, parent, time_offset, microwave, microwave_cmd,
        ...
        self.microwave_cmd      = microwave_cmd
        self.microwave_powers   = microwave_powers
        self.microwave_power_low= float(microwave_power_low)
        self.microwave_frequency= float(microwave_frequency)
        self.microwave_time_step= float(microwave_time_step)
        self.laser              = laser
        self.laser_cmd          = laser_cmd
        self.laser_powers       = laser_powers
        self.wait_time          = float(wait_time)

        if isinstance(self.microwave_powers, str):
            self.microwave_powers = eval(self.microwave_powers)
        if isinstance(self.laser_powers, str):
            self.laser_powers = eval(self.laser_powers)

        self.verification_string = 'sweep'
        self.warnings = []
        self.new_attributes = []
