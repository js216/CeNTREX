        ...
        try:
            if func(*args, **kwargs):
                ...
                if not b"\x01\x35\xff\xff" == ret:
                    warning_dict = {
                        "message": "invalid response in {0}".format(func.__name__)
                    }
                    args[0].warnings.append([time.time(), warning_dict])
                    logging.warning(
                        "SelfAlignFiberSwitch warning in {0}() : ".format(func.__name__)
                        + "invalid response"
                    )
...
class SelfAlignFiberSwitch:
    ...
    def SetPort(self, port):
        ...
        if (port > 16) or (port < 1):
            logging.warning(
                "SelfAlignFiberSwitch warning in Setport({0}) : ".format(port)
                + "port out of range"
            )
            return
