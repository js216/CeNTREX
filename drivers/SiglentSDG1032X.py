...
def validate_channel(func):
    def wrapper(*args, **kwargs):
        if args[1] in [1, 2]:
            ...
            logging.warning(
                f"SiglentSDG1032X warning in {func.__name__}: invalid channel"
            )
...
class SiglentSDG1032X:
    ...
    def SetBurstMicrowaveDelay(self, freq, phase_offset=60):
        ...
        if not np.isclose(
            delay1,
            float(self.instr.query("C1:BTWV?")[8:].split(",")[9].strip("S")),
            atol=1e-9,
        ):
            logging.warning(
                "SiglentSDG1032X warning in BurstMicrowaveDelay: ch1 delay not set"
            )
        if not np.isclose(
            delay2,
            float(self.instr.query("C2:BTWV?")[8:].split(",")[9].strip("S")),
            atol=1e-9,
        ):
            logging.warning(
                "SiglentSDG1032X warning in BurstMicrowaveDelay: ch2 delay not set"
            )
        return
    ...
    def OutputImpedance(self, ch: int, impedance: str):
        if impedance in [50, "50", "HZ"]:
            ...
            logging.warning(
                f"SiglentSDG1032X warning in OutputImpedance: invalid impedance"
            )
    ...
    def Square(
        ...
        cmd = (
            f"C{ch}:BSWV"
            f" WVTP,SQUARE,FRQ,{freq},AMP,{amp},OFST,{offset},PHSE,{phase},DUTY,{duty}"
        )
        self.instr.write(cmd)
        self.ParseBasicWave(ch)
    ...
    def Pulse(
        ...
        cmd = (
            f"C{ch}:BSWV"
            f" WVTP,PULSE,FRQ,{freq},AMP,{amp},OFST,{offset},DLY,{delay},DUTY,{duty}"
        )
        self.instr.write(cmd)
        self.ParseBasicWave(ch)
    ...
    def BurstWave(
        ...
        cmd = (
            f"C{ch}:BTWV"
            f" STATE,{state},GATE_NCYC,{gate_ncyc},TRSR,{trigger},DLY,{delay},PLRT,{polarity},"
        )
        cmd += f"EDGE,{edge}"
        self.instr.write(cmd)
