...
class SRSPS350:
    ...
    def GetWarnings(self):
        ...
        for val in stb:
            if val in [1,2,3,5]:
                logging.warning(f'SRS PS350 warning : {self.serial_poll_status_byte[val]}')
    ...
    def _CheckCommandExecution(self, cmd):
        ...
        for val in esr:
            if val in [2,3,4,5,6,7]:
                logging.warning(f'SRS PS350 warning in {cmd} : {self.standard_event_status_byte[val]}')
    ...
    def SetHV(self, state):
        if state in [True, False]:
            state = 'HVON' if state else 'HVOF'
            self.instr.write(f"{state}")
        else:
            logging.warning(f"SRS PS350 warning in SetHV(): set state True (On) or False (Off) != {state}")

    def GetILim(self):
        return float(self.instr.query('ILIM?'))

    @CheckCommandExecution
    def SetILim(self, current):
