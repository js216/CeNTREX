...
class Monitoring(threading.Thread, PySide6.QtCore.QObject):
    # signal to update the style of a QWidget
    update_style = PySide6.QtCore.Signal(qt.QWidget)
            ...
            for dev_name, dev in self.parent.devices.items():
                ...
                if len(dev.warnings) != 0:
                    ...
                    for warning in dev.warnings:
                        ...
                        if self.parent.config.influxdb.enabled > 0:
                            self.push_warnings_to_influxdb(dev.config, warning)
                        self.parent.ControlGUI.update_warnings(warning.to_text())
                    dev.warnings = []
                ...
                for c_name, params in dev.config.control_params.items():
                    if params.type in [
                        ControlParamTypes.INDICATOR,
                        ControlParamTypes.INDICATORBUTTON,
                        ControlParamTypes.INDICATORLINEEDIT,
                    ]:
                ...
                try:
                    if dev.config.slow_data:
                        formatted_data = [
                            ...
                            for x in get_values_from_slow_device_dataclass(data)
                        ]
                    else:
                        formatted_data = [
                            np.format_float_scientific(x, precision=3)
                            for x in data.data[0, :, 0]
                        ]
                ...
                if (
                    time.time() - self.time_last_monitored
                    >= self.parent.config.general.monitoring_dt
                ):
    ...
    def write_to_influxdb(self, dev: DeviceProtocol, data):
        ...
        if not self.parent.config.influxdb.enabled:
            return
        if not dev.config.control_params["InfluxDB_enabled"].value:
            return
        ...
        try:
            _fields = []
            for idk, key in enumerate(zip(dev.col_names_list[1:])):
        ...
        p = p.time(
            time=datetime.datetime.utcfromtimestamp(
                data.time + self.parent.config.time_offset
            ).isoformat()
        )
    ...
    def push_warnings_to_influxdb(
        self, dev_config: DeviceConfig, warning: DeviceWarning
    ) -> None:
        json_body = [
            {
                ...
                "time": int(1000 * warning.time),
                "fields": {"message": warning.message, "level": warning.level.name},
            }
        ]
        self.write_api.write(
