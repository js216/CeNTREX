...
def generate_repeating_pulses(pulses, masking_pulses, duration=None):
    ...
    for pulse in masking_pulses:
        ...
        if o != 0:
            instruction_cycles.append(o)
        masking_pulses_cycle_units.append(
            {"frequency": f, "offset": o, "high": h, "channels": pulse["channels"]}
        )
    ...
    for idx in range(nr_cycles):
        ...
        for idx_pulse, pulse in enumerate(pulses_cycle_units):
            if idx >= pulse["offset"]:
                ...
                idx_reset_pulse[idx_pulse] = (
                    idx_reset_pulse[idx_pulse] % pulse["frequency"]
                )
        ...
        for idx_pulse, pulse in enumerate(masking_pulses_cycle_units):
            if idx >= pulse["offset"]:
                ...
                idx_reset_masking_pulse[idx_pulse] = (
                    idx_reset_masking_pulse[idx_pulse] % pulse["frequency"]
                )
        if channels_active:
            chs = [0] * 24
            chs[-3:] = [1, 1, 1]
    ...
    channels_inverted = [
        p for pulse in pulses for p in pulse["channels"] if not pulse["active_high"]
    ]
    ...
    pulse_sequence.append(
        {
            "label": "",
            "channels": channels_off.copy(),
            "time": min_instruction_len,
            "opcode": "BRANCH",
            "instdata": 0,
        }
    )
...
class PulseBlaster:
    def __init__(
        ...
        try:
            ...
            if pb_init() != 0:
                ...
                logging.warning(
                    "PulseBlaster error in itial connection : could not initialize"
                )
                self.__exit__()
    ...
    def GenerateSequenceQSwitch(self, qswitch_delay):
        ...
        shutter = {
            "frequency": 5,
            "offset": int(qswitch_delay * 1e3) + int(70e-3 / 1e-9) + 1,
            "high": int(100e-3 / 1e-9),
            "channels": [3, 4],
            "active_high": True,
        }
    ...
    def ProgramDevice(self, board_number=None):
        ...
        if not self.sequence:
            logging.warning(
                "PulseBlaster warning in ProgramDevice: no sequence supplied"
            )
        ...
        try:
            for seq in sequence:
                ...
                if opcode.upper() == "JSR":
                    try:
                        pb_inst_pbonly(
                            channels, JSR, addresses[instdata], duration * ns
                        )
                    except KeyError:
                    ...
                    try:
                        pb_inst_pbonly(
                            channels, BRANCH, addresses[instdata], duration * ns
                        )
                    except KeyError:
                    ...
                    try:
                        pb_inst_pbonly(
                            channels, END_LOOP, addresses[instdata], duration * ns
                        )
                    except KeyError:
...
if __name__ == "__main__":
    ...
    shutter = {
        ...
        "offset": int(trace_length * 1e6 + 1e6),
